pipeline {
    ...
    environment {
        AWS_ACCESS_KEY_ID     = credentials('terrafrom-aws-tcs-Jenkins')
        AWS_SECRET_ACCESS_KEY = credentials('terrafrom-aws-tcs-Jenkins')
    }
    ...
    stages {
        ...
        stage('Approval') {
            ...
            steps {
                script {
                    sh 'terraform show tfplan.txt > tfplan-readable.txt'
                    def plan = readFile('tfplan-readable.txt')
